Let \((M,g)\) be an oriented connected Riemannian \(n\)-manifold and denote by \(\omega_g \in \Omega^n(M)\) the corresponding Riemannian volume form. 
...
\begin{split}
    ...
    \overline{\eta} &= \eta_{j_1 \dots j_k} e^{j_1} \otimes \cdots \otimes e^{j_k}
\end{split}
\]
(and the indices are not ordered in this sum).


We can identify \((\Lambda^kT^*_pM)^*=\Lambda^kT_pM\) with \(\Lambda^kT^*_pM\) by lowering every index, that is via the musical isomorphism
\begin{align*}
    \sharp \colon \Lambda^kT^*_pM &\to (\Lambda^{k}T^*_pM)^*=\Lambda^kT_pM \\
...
\begin{proof}
    ...
    \[
        \star (\dif x^{i_1} \wedge \cdots \wedge \dif x^{i_k}) = \sqrt{\det(g_{ij})}\sigma(i_1 \dots i_k j_1 \dots j_{n-k})\dif x^{j_1} \wedge \cdots \wedge \dif x^{j_k}
    \]
    where
    \[
    ...
    \[
    \begin{split}
        \star \star (\dif x^{i_1} \wedge &\cdots \wedge \dif x^{i_k}) \\
        &= \sqrt{\det(g_{ij})}{(-1)^{1+\cdots+n-(1+\cdots+k) - (1 +\cdots+(n-k))}}\dif x^{i_1} \wedge \cdots \wedge \dif x^{i_k}.
    \end{split}
    \]
    Then observe that
    \[        
