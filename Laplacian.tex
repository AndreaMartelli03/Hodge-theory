Assume that \((M,g)\) is closed (compact and without boundary). From now on, we will omit the Riemannian volume form when integrating functions, that is, we write
\[
...
\begin{defn}
    The \emph{connection Laplacian} the second order differential operator \(\nabla^*\nabla\) defined by
    \[
...
\[
\begin{split}
    (\triangle \omega,\eta)_{L^2}&=\int_M g(\triangle \omega, \eta) = \int_M g(\dif \omega, \dif \eta) + \int_M g(\delta \omega,\delta g)\\
...
\begin{lemma}[Weitzenb\"ock identity]\label{lemma: Weitzenbock identity}
    ...
    \]
    is the \emph{Weitzenböck curvature operator}\footnote{
