...
Denote also 
\[
    \Hcal^k(M) \coloneq \ker \dif|_{\Omega^k} \cap \ker \delta|_{\Omega^k}, \qquad 
    \Hcal^k_D(M) \coloneq \Hcal^k \cap \Omega^k_D, \qquad 
    \Hcal^k_N(M) \coloneq \Hcal^k \cap \Omega^k_N.
\]
...
\begin{prop}\label{prop: fredholm}
    ...
    i.e., 
    \[
        \int_M g(\theta, \eta) = 0 \qquad \forall \eta \in \mathcal{X} \text{ s.t. } \triangle\eta= 0.
    \]
    In that case, the solution is unique.
\end{prop}
\begin{rmk}
    Observe that \(\Hcal^k_D(M) \subset \Hcal^k_\triangle(M)\).
\end{rmk}
...
\begin{thm}\label{thm: Hodge - de Rham with Dirichlet BC}
    The natural map \(\Hcal^k_D(M) \to H^k_{dR}(M,\de M)\) is a linear isomorphism. In other words, every de Rham cohomology class relative to the boundary is represented by a unique harmonic form satisfying the homogeneous Dirichlet boundary condition.
\end{thm}
\begin{proof}
    ...
    \]
    so actually \(\omega = \theta + \dif \eta + \delta \beta\). But then \(\delta \beta \in \Hcal^k_D\), therefore \(\delta \beta = 0\). Thus, \(\omega = \theta + \dif \eta\) with \(\eta \in \Omega^{k-1}_D(M)\), i.e., \([\omega]=[\theta]\).
\end{proof}
